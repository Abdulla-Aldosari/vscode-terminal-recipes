...
| ---------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1          | `media/state.js`               | All mutable UI state + app data received from extension. `state.workspaceFolders` holds `Array<{ name, fsPath }>` received from the extension — populated only in multi-root workspaces. `runConfirmState.selectedFsPath` holds the folder path the user chose in the run-confirm folder dropdown for the current execution (transient, not persisted).                                                                                                                                                                                                                                                                          | `vscode`, `RECIPES_EMPTY_VALUE`, `uiState`, `state`, `variableInputState`, `runConfirmState`, `aiExplainState`                                                                                                                                                                                                                              |
...
| 4          | `media/modals/run-confirm.js`  | Run Confirmation modal + Variable Input modal + Delete Confirm modal. In multi-root workspaces: `renderRunConfirmFolderSelector()` adds a folder dropdown to the modal so the user can override the workspace folder for this specific execution; `withRunConfirmFolderContext(fn)` temporarily patches `state.autoVariables` so `${workspaceFolder}` and `${workspaceName}` in the preview resolve against the selected folder before calling `highlightResolvedHtml`. The `.modal-command-preview::before` CSS property uses the CSS variable `--cmd-preview-prefix` (default `"> "`, set to `"FolderName > "` in multi-root). | `renderToggleSwitch3`, `renderRunConfirmModal`, `renderRunConfirmFolderSelector`, `withRunConfirmFolderContext`, `renderVariableInputModal`, `renderDeleteConfirmModal`                                                                                                                                                                     |
...
| 17         | `media/main.js`                | Entry point: `DOMContentLoaded` init + all `postMessage` senders + tooltip IIFE. `postSetActiveWorkspaceFolder(fsPath)` sends the user's folder selection to the extension.                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `postReady`, `postSaveData`, `postSaveCommandVariables`, `postPerformAction`, `postAiGenerate`, `postSetActiveWorkspaceFolder`, ...                                                                                                                                                                                                         |
...
| `postSetActiveWorkspaceFolder(fsPath)`   | `setActiveWorkspaceFolder`  | dispatch table in `extension.js` — persists `fsPath` in `context.workspaceState` and calls `postState()` |
...
| **Run Confirm Modal** — workspace folder (multi-root only) | `run-confirm-folder-select` | `media/modals/run-confirm.js` — visible only when `state.workspaceFolders.length > 1`; overrides the workspace folder used for this specific execution |
