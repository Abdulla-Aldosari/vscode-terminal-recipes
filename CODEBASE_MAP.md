...
| File                                 | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `lib/ai/providers-config.js`         | **SINGLE SOURCE OF TRUTH** — metadata for all AI providers (name, displayLabel, URL, setup steps, and per-provider `models[]` array with `modelId`, `modelLabel`, `free` — supports multiple models per provider). Edit this first when adding a new provider.                                                                                                                                                                       |
| `lib/ai/factory.js`                  | `generateWithAI(options)` — main entry point; accepts `shellName?` to inject a shell-context section into the system instruction via `buildShellContext(shellName)`. `createProvider(name, key, modelId)` — instantiates the correct provider class. `listModelsForProvider(providerName, apiKey)` — calls the provider's `listModels()` and returns a normalized `{ modelId, modelLabel }[]` array. Edit to register new providers. |
...
| `lib/ai/providers/bytedancesseed.js` | ByteDance Seed implementation (OpenAI-compatible, `ark.cn-beijing.volces.com/api/v3`)                                                                                                                                                                                                                                                                                                                                                |
| `lib/ai/providers/stepfun.js`        | StepFun implementation (OpenAI-compatible, `api.stepfun.com/v1`)                                                                                                                                                                                                                                                                                                                                                                     |
...
| 7          | `media/modals/ai-settings.js`  | AI Settings modal (provider selector + cascading model selector + API key) + localStorage model cache helpers | `renderAiSettingsModal`, `renderAiProviderSetupModal`, `getAiModelLabel`, `buildModelOptions`, `resolveSettingsModelId`, `getCachedModels`, `setModelsCache`, `getModelsCacheFetchedAt`, `formatTimeAgo`                                                                                                                                    |
| 8          | `media/modals/ai-generate.js`  | AI Generate prompt, loading overlay, results modal, and all AI bind events                                    | `renderAiPromptModal`, `renderAiLoadingOverlay`, `renderAiResultsModal`, `bindAiEvents`                                                                                                                                                                                                                                                     |
...
| `aiListModelsResult`              | inline in message dispatcher            | Saves result to localStorage cache; applies models to `aiState.aiProviderSetup[provider].models`; re-renders only if result is for the currently displayed provider |
...
| `btn-ai-settings-save`   | Save the API key to VS Code `SecretStorage` and persist the selected model ID to VS Code settings (`terminalRecipes.aiModel`)      |
...
| `btn-ai-refresh-models`  | Fetch the latest model list for all providers that have a saved API key (bound in `bindAiEvents` in `media/modals/ai-generate.js`) |
