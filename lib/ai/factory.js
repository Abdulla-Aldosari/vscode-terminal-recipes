...
async function generateWithAI(options) {
  const { providerName, modelId, apiKey, prompt, mode, customSystemInstruction, categoryId, groupId, shellName } =
    options;
  ...
  try {
    ...
    finalResult = mode === "full" ? remapFullModeIds(aiResult) : remapSingleModeResult(aiResult, categoryId, groupId);
  } catch (err) {
    logAiInteraction({ provider: providerName, mode, prompt, systemInstruction, error: err });
    throw err;
  }

  logAiInteraction({ provider: providerName, mode, prompt, systemInstruction, schema, result: finalResult });
  return finalResult;
}
...
async function listModelsForProvider(providerName, apiKey) {
  ...
  const seen = new Set();
  const models = raw.filter(function (m) {
