...
/**
 * AI provider implementation for ByteDance Seed.
 * Uses the Volcano Engine Ark OpenAI-compatible endpoint.
 */
class ByteDanceSeedProvider {
  constructor(apiKey, modelId) {
    this.client = new OpenAI({
      apiKey,
      baseURL: 'https://ark.cn-beijing.volces.com/api/v3',
    });
    const cfg = getProviderConfig('bytedancesseed');
...
Do NOT include any text, explanation, or markdown before or after the JSON.`;
    ...
    if (logger) {
      logger.appendLine(formatRequestLog('bytedancesseed', mode, enhancedSystem, prompt, schema));
      logger.show(true);
    }
    ...
    if (!jsonMatch) {
      throw new Error('ByteDance Seed response did not contain valid JSON.');
    }
...
module.exports = {ByteDanceSeedProvider};
