...
function resolveActiveWorkspaceFolder(savedFsPath) {
  ...
  const resolution =
    vscode.workspace
      ...
      .get("multiRootFolderResolution") || "remember";
  ...
  function isValidFolder(fsPath) {
    return Boolean(fsPath && folders.some(function (f) { return f.uri.fsPath === fsPath; }));
  }
  ...
  if (resolution === "followEditor") {
    ...
    return getEditorFolder()
      || (isValidFolder(savedFsPath) ? savedFsPath : null)
